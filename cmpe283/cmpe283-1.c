...
/*
  * Procbased Controls capabilities
  * See SDM volume 3, section 23.6.2
 */
struct capability_info procpbased[22] =
{
...
/*
 *  Procbased Control2 capabilities	
 * See SDM volume 3, section 23.6.2
 */
struct capability_info procsbased[27] =
{
...
detect_vmx_features(void)
{
    ...
    pr_info("ProcBased Controls MSR: 0x%llx\n",
    	(uint64_t)(lo | (uint64_t)hi << 32));
    report_capability(procpbased, 22, lo, hi);
    
   /*  Procbased controls two */
  	rdmsr(IA32_VMX_PROCBASED_CTLS2, lo, hi);
  	pr_info("Secondary ProcBased Controls MSR: 0x%llx\n",
  		(uint64_t)(lo | (uint64_t)hi << 32));
  	report_capability(procsbased, 27, lo, hi);  
